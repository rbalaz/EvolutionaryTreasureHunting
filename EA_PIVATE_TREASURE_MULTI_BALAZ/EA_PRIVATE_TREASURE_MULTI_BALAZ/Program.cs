...
namespace PirateTreasure
{
    class Program
    {
        ...
        static void SaveToFile(List<Utils.Path> bestPathsPerGeneration)
        {
            ...
            for (int i = 0; i < bestPathsPerGeneration.Count; i = i + 6)
            {
                writer.WriteLine("# generacia " + (i / 6 + 1));
                for (int j = 0; j < bestPathsPerGeneration[i].Points.Count; j++)
                {
                    ...
                    writer.Write(bestPathsPerGeneration[i + 1].Points[j].Y.ToString().PadLeft(2) + " ");
                    writer.Write(bestPathsPerGeneration[i + 2].Points[j].X.ToString().PadLeft(2) + " ");
                    writer.Write(bestPathsPerGeneration[i + 2].Points[j].Y.ToString().PadLeft(2) + " ");
                    writer.Write(bestPathsPerGeneration[i + 3].Points[j].X.ToString().PadLeft(2) + " ");
                    writer.Write(bestPathsPerGeneration[i + 3].Points[j].Y.ToString().PadLeft(2) + " ");
                    writer.Write(bestPathsPerGeneration[i + 4].Points[j].X.ToString().PadLeft(2) + " ");
                    writer.Write(bestPathsPerGeneration[i + 4].Points[j].Y.ToString().PadLeft(2) + " ");
                    writer.Write(bestPathsPerGeneration[i + 5].Points[j].X.ToString().PadLeft(2) + " ");
                    writer.WriteLine(bestPathsPerGeneration[i + 5].Points[j].Y.ToString().PadLeft(2));
                }
                writer.WriteLine();
                writer.WriteLine();
            }
